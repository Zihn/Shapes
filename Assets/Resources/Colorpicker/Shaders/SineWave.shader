// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'

// Upgrade NOTE: replaced '_Object2World' with 'unity_ObjectToWorld'

// Upgrade NOTE: replaced '_CameraToWorld' with 'unity_CameraToWorld'
// Upgrade NOTE: replaced '_World2Object' with 'unity_WorldToObject'

Shader "ColorPicker/SineWave" {
	Properties {
	      _XPosition ("x offset rectTransform", Float) = 0.0
	      _YPosition ("y offset rectTransform", Float) = 0.0
	      _Width ("width rectTransform", Float) = 1.0
	      _Height ("height rectTransform", Float) = 1.0
	      _Rotation	("Orientation of the shader in rad", Float) = 0.0
	      _T ("Time", Float) = 0.0
	}
	SubShader {
	    Pass {	
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag
			#include "UnityCG.cginc"
			#define M_PI 3.1415926535897932384626433832795		
			uniform float _XPosition;
			uniform float _YPosition;
			uniform float _Rotation;
			uniform float _Width;
			uniform float _Height;
			uniform float _T;
	
			struct pos_output {
			    float4 pos : SV_POSITION;
			    float4 wp : pos;
			};
			
			pos_output vert(appdata_full v) {
			    pos_output o;
			    o.pos = UnityObjectToClipPos(v.vertex);
			    o.wp = mul(unity_ObjectToWorld, v.vertex);
			    return o;
			}
			
			half4 frag(pos_output o) : COLOR {

			float xPos = (o.wp.x - _XPosition)/_Width;
			float yPos = (o.wp.y - _YPosition)/_Height;
			float A1 = 0.50;
			float B1 = 0.25;
			float B2 = 4.0;
			float B3 = 4.0;
			float C1 = 1.0;
			float C2 = 2.0;
			float C3 = 1.0;
			float D1 = 0.10;
			float D2 = 3.0;
			float D3 = 5.0;
			float E1 = 1.0;
			float E2 = 5.0;
			float E3 = 3.0;
			float F1 = 0.15;
			float F2 = 1.0;
			float F3 = 0.6;

			half v = A1 + B1 * sin(B2 * M_PI * xPos + B3 * _T) * C1 * sin(C2* M_PI * yPos + C3 * _T) +
			D1 * cos(D2 * M_PI * xPos + D3 * _T) * E1 * cos(E2 * M_PI * yPos + E3 * _T) +
			F1 * sin(F2 * M_PI * xPos + F3 * _T);
//				half ang = atan2(o.wp.y - _YPosition, o.wp.x - _XPosition) + _Rotation + M_PI;
//				ang = (ang > 0 ? ang : (2*M_PI + ang));
//				ang = (ang < 2*M_PI ? ang : ang - 2*M_PI);
//				ang = ang / (2*M_PI);
				half p = floor(v*6);
				half i = v*6-p;
				half4 c = p == 0 ? half4(1, i, 0, 1) :
						  p == 1 ? half4(1-i, 1, 0, 1) :
						  p == 2 ? half4(0, 1, i, 1) :
						  p == 3 ? half4(0, 1-i, 1, 1) :
						  p == 4 ? half4(i, 0, 1, 1) :
						  p == 5 ? half4(1, 0, 1-i, 1) :
						           half4(1, 0, 0, 1);
			    return c;
			}
			ENDCG
	    }
	}
}

